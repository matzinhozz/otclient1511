...
function setStringColor(tooltipTable, text, color)
    table.insert(tooltipTable, {text = text, color = color})
end
...
function PartyHuntAnalyser:updateWindow(updateMembers, ignoreVisible)
    ...
    for _, member in ipairs(PartyHuntAnalyser.membersData) do
        local id = member.id
        ...
        lootTotal = lootTotal + member.loot
        supplyTotal = supplyTotal + member.supply
        local playerBalance = member.loot - member.supply
        local playerName = PartyHuntAnalyser.membersName[id] or "Unknow"

        widget.name:setText(playerName)
        if not member.highlight or member.highlight == 0 then
            widget.name:setColor("gray")
        end
        widget.balance:setText(comma_value(playerBalance))
        widget.balance:setColor(playerBalance >= 0 and "green" or "orange")
        widget.damage:setText(comma_value(member.damage))
        widget.healing:setText(comma_value(member.healing))
        widget:setId(id)

        local tooltipMessage = {}
        setStringColor(tooltipMessage, tr("Loot: %s ", comma_value(member.loot)), "#3f3f3f")
        setStringColor(tooltipMessage, "$\n", "yellow")
        setStringColor(tooltipMessage, tr("Supplies: %s ", comma_value(member.supply)), "#3f3f3f")
        setStringColor(tooltipMessage, "$\n", "yellow")
    ...
    contentsPanel.balance:setColor(balance >= 0 and "green" or "orange")
end
