...
function mapController:onGameStart()
    ...
    -- Sprawdź czy LocalPlayer jest dostępny przed rejestracją eventów
    local player = g_game.getLocalPlayer()
    if player then
        mapController:registerEvents(player, {
            onPositionChange = onPositionChange
        }):execute()
    else
        -- Jeśli LocalPlayer nie jest dostępny, spróbuj ponownie później
        addEvent(function()
            local player = g_game.getLocalPlayer()
            if player then
                mapController:registerEvents(player, {
