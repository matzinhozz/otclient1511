...
local silent
local protocol
-- coalescing of browse requests to avoid duplicates on rapid UI changes
local browseEvent
local pendingSig
...
-- Allow external code to cancel any pending coalesced browse
...
function MarketProtocol.sendMarketBrowse(browseId, itemId, tier)
    if g_game.getFeature(GamePlayerMarket) then
        ...
        local it = tonumber(itemId) or 0
        local tr = tonumber(tier) or 0
        local sig = string.format('%d:%d:%d', id, it, tr)
        ...
        browseEvent = scheduleEvent(function()
            ...
            if g_game.getClientVersion() >= 1251 then
                msg:addU8(id)
                msg:addU16(it)
                if g_game.getFeature(GameThingUpgradeClassification) and tr > 0 then
                    msg:addU8(tr)
                end
            else
                msg:addU16(it)
